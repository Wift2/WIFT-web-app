...
interface CreateFloorplanPageProps {
  router: Router;
}

const CreateFloorplanPage = ({ router }: CreateFloorplanPageProps) => {
  ...
  useEffect(() => {
    const urlParams = router.searchParams;
    ...
    if (formType === 'squareFootage') {
      ...
      const seatingType = urlParams.get('seatingType');
      const tableSize = urlParams.get('tableSize');
      const tableSeats = urlParams.get('tableSeats');

      // Create custom venue based on form data
      if (length && width && height) {
        const customVenue: Venue = {
          id: 'custom-square-footage',
          name: `Custom Space (${length}'×${width}'×${height}')`,
          dimensions: {
            width: Number.parseInt(width) * 10,
            height: Number.parseInt(length) * 10,
          }, // Scale for display
          capacity: Math.floor(
            (Number.parseInt(length) * Number.parseInt(width)) / 20
          ), // Rough estimate
          description: `Custom space created from square footage with ${screenType} screen and ${seatingType} seating`,
          features: [
            `${screenType} Screen`,
            `${seatingType} Seating`,
            `${tableSize}" Tables with ${tableSeats} seats`,
            'Custom Layout',
          ],
        };
        ...
        setFloorplanName(
          `${customVenue.name} - ${new Date().toLocaleDateString()}`
        );
        setIsGenerating(true);

        // Generate layout after venue is set
        setTimeout(() => {
          setFloorplanElements(generateInitialLayout(customVenue));
    ...
    } else if (formType === 'attendees') {
      const attendees = urlParams.get('attendees');
      const screenType = urlParams.get('screenType');
      const seatingType = urlParams.get('seatingType');

      if (attendees) {
        const attendeeCount = Number.parseInt(attendees);
        // Estimate dimensions based on attendee count
        const estimatedArea = attendeeCount * 20; // 20 sq ft per person
        const estimatedWidth = Math.ceil(Math.sqrt(estimatedArea));
        const estimatedLength = Math.ceil(estimatedArea / estimatedWidth);

        const customVenue: Venue = {
          id: 'custom-attendees',
          name: `Custom Space for ${attendees} Attendees`,
          dimensions: {
            width: estimatedWidth * 10,
            height: estimatedLength * 10,
          }, // Scale for display
          capacity: attendeeCount,
          description: `Custom space designed for ${attendees} attendees with ${screenType} screen and ${seatingType} seating`,
          features: [
            `${screenType} Screen`,
            `${seatingType} Seating`,
            `Capacity: ${attendees} people`,
            'Optimized Layout',
          ],
        };
  ...
  // Select Large Auditorium by default on component mount (only if no custom form data)
  useEffect(() => {
    if (!selectedVenue) {
